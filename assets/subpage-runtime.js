...
(function(){
  ...
  ids.forEach(id=>{
    ...
    const el = /Search/.test(id) ? document.createElement('input') : /clear|jump|reset|Close/.test(id) ? document.createElement('button') : document.createElement('div');
    el.id = id;
    host.appendChild(el);
  });
  ...
  function lockBottomNav(){
    ...
    if(window.innerWidth >= 700) nav.style.setProperty('padding-bottom','7px','important');
  }
  lockBottomNav();
